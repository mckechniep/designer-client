...
begin
  if not exists (
    ...
    create type public.app_user_role as enum ('client', 'admin');
  end if;
  ...
  if not exists (
    ...
    create type public.app_generation_status as enum ('queued', 'running', 'succeeded', 'failed');
  end if;
  ...
  if not exists (
    ...
    create type public.app_asset_kind as enum ('master_background', 'splash', 'button_preview', 'thumbnail', 'download_package');
  end if;
end $$;
...
create table if not exists public.design_directions (
  id uuid primary key default gen_random_uuid(),
  project_id uuid not null references public.projects(id) on delete cascade,
  title text not null,
...
create table if not exists public.asset_generations (
  ...
  status public.app_generation_status not null default 'queued',
...
create table if not exists public.asset_files (
  ...
  kind public.app_asset_kind not null,
...
create table if not exists public.style_specs (
  id uuid primary key default gen_random_uuid(),
  generation_id uuid not null references public.asset_generations(id) on delete cascade,
...
create index if not exists user_profiles_client_profile_id_idx on public.user_profiles(client_profile_id);
create index if not exists projects_client_profile_id_idx on public.projects(client_profile_id);
create unique index if not exists design_briefs_project_id_key on public.design_briefs(project_id);
create index if not exists design_directions_project_id_idx on public.design_directions(project_id);
create unique index if not exists design_directions_one_selected_per_project_idx
  on public.design_directions(project_id)
  where is_selected;
create index if not exists asset_generations_direction_id_idx on public.asset_generations(design_direction_id);
create index if not exists asset_files_generation_id_idx on public.asset_files(generation_id);
...
alter table public.user_profiles alter column role set default 'client'::public.app_user_role;

alter table public.asset_generations alter column status drop default;
alter table public.asset_generations
  alter column status type public.app_generation_status
  using status::text::public.app_generation_status;
alter table public.asset_generations alter column status set default 'queued'::public.app_generation_status;

alter table public.asset_files
  alter column kind type public.app_asset_kind
  using kind::text::public.app_asset_kind;

create schema if not exists private;
revoke all on schema private from public;
grant usage on schema private to authenticated;

drop policy if exists "admins and clients can read client profiles" on public.client_profiles;
drop policy if exists "users can read own profile" on public.user_profiles;
drop policy if exists "admins and clients can read generation limits" on public.generation_limits;
drop policy if exists "admins can insert generation limits" on public.generation_limits;
drop policy if exists "admins can update generation limits" on public.generation_limits;
drop policy if exists "admins can delete generation limits" on public.generation_limits;
...
drop policy if exists "clients can read own briefs" on public.design_briefs;
drop policy if exists "clients can insert own briefs" on public.design_briefs;
drop policy if exists "clients can update own briefs" on public.design_briefs;
drop policy if exists "clients can read own directions" on public.design_directions;
drop policy if exists "clients can read own generations" on public.asset_generations;
drop policy if exists "clients can read own asset files" on public.asset_files;
drop policy if exists "clients can read own style specs" on public.style_specs;
drop policy if exists "clients can insert own download events" on public.download_events;
drop policy if exists "clients can read own download events" on public.download_events;
...
drop function if exists private.current_client_profile_id();
